...
export const BriefSchema = z.object({
  ...
  goal: z
    ...
    .max(2000)
    .describe(
      "The user's objective in the assistant's words, AND the headline a client displays for this plan. ONE scannable line (aim ≤ ~60 chars) — put coverage, rationale, and constraints in preferences or a recommendation's why, never in goal",
    ),
  /** Consumption time available for this plan (briefs use minutes; SPEC.md §2). */
  available_minutes: z.number().int().positive().optional(),
  knowledge_level: KnowledgeLevelSchema.optional().describe(
    "OMIT unless the user stated their level or their words clearly imply " +
