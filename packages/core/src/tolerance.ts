...
export function tolerantEnum<T extends [string, ...string[]]>(
  schema: z.ZodEnum<T>,
): z.ZodType<Tolerant<T[number]>> {
  // The fallback is for enum values a future minor ADDED — machine-ish slugs,
  // so the 0.4 bounds pass caps it at 100 like any other slug-class string.
  return z.union([schema, z.string().max(100)]) as unknown as z.ZodType<Tolerant<T[number]>>;
}

/** Narrow a tolerantly-parsed value back to the enum's known literals. */
export function isKnown<T extends [string, ...string[]]>(
  schema: z.ZodEnum<T>,
  value: string,
): value is T[number] {
  return (schema.options as readonly string[]).includes(value);
}
