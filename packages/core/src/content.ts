...
export const KNOWN_CONTENT_TYPES: readonly string[] = ContentTypeSchema.options;
...
const PODCAST_REF_IDENTIFIER_MESSAGE =
  "podcast_episode ContentRef must include at least one of episode_id, guid, feed_url, episode_url";

function hasEpisodeIdentifier(ref: z.infer<typeof podcastEpisodeRefObject>): boolean {
  return (
    ref.episode_id !== undefined ||
    ref.guid !== undefined ||
 ...
 */
export function isPodcastEpisodeRef(ref: ContentRef): ref is PodcastEpisodeRef {
  return ref.content_type === "podcast_episode";
}

/** Narrow a ContentRef to the (schema-only) article variant. */
export function isArticleRef(ref: ContentRef): ref is ArticleRef {
...
export function isResolvedPodcastEpisode(
  content: ResolvedContent,
): content is ResolvedPodcastEpisode {
